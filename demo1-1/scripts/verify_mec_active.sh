#!/bin/bash
#
# verify_mec_active.sh
#
# Purpose: Script to be run on MEC PC.
#          Verifies that the MEC server docker container is attached to the
#          N6 (oai-traffic-net) network, has the correct IP address, is
#          listening on port 8080 of the MEC container, and is ready to
#          service clients.
#
# Prerequisites:
...
for container in "$UPF_CONTAINER" "$MEC_CONTAINER"; do
...
[[ -n "$LISTEN_OUTPUT" ]] ||
    die "MEC server is not listening on port 8080"
