...
Application code and configuration live in `src/`, and Docker build and Compose
files are in `docker/`. Run the application
commands below from `src/`. For the container workflow, run
`make prepare-mec` from `dist-sparks/`, then `./scripts/start_mec.sh` after SRK starts.
